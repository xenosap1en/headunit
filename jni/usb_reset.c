...
int usb_reset(char *usb_device)
{
	...
	{
		logd("open errno: %d (%s)\n", errno,
		     strerror(errno)); // open errno: 2 (No such file or directory)  !! Reset changes directory entries for "/data/data/ca.yyx.hu/lib/libusb_reset.so /dev/bus/usb/*/*"
		return (0);	    //-1);
	}
	...
	if (ret < 0)
	{
		logd("ioctl errno: %d (%s)\n", errno,
		     strerror(errno)); // ioctl errno: 21 (Is a directory)         !! Reset changes directory entries for "/data/data/ca.yyx.hu/lib/libusb_reset.so /dev/bus/usb/*/*"
		return (0);	    //-2);
	}
...
/*
    ...
    unsigned long sleep_val = 1L;
    for (sleep_val = 1L; sleep_val <= 10000L; sleep_val *= 2L)
      logd ("ms_sleep (%ld): %5.5d  errno: %3.3d (%32.32s)  ms_get: %16ld", ms_sleep (sleep_val), sleep_val, errno, strerror (errno), ms_get ());    // ret: 00000  errno: 000 (
Success)  us_get:         11358714
...
///*
    ...
    for (tm.tv_nsec = 1; tm.tv_nsec <= 100000000L; tm.tv_nsec *= 10)
      logd ("nanosleep (%ld): %5.5d  errno: %3.3d (%32.32s)  us_get: %16ld", nanosleep (& tm, NULL), tm.tv_nsec, errno, strerror (errno), us_get ());    // ret: 00000  errno: 000 (
Success)  ms_get:         11358714
return (0);
//*/
