...
int     LIBUSB_CALL libusb_control_transfer       (libusb_device_handle *dev_handle, uint8_t request_type, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, unsigned char *data, uint16_t wLength,
...
#define libusb_control_transfer       usb_device_control_transfer
    #define libusb_bulk_transfer          usb_device_bulk_transfer        int                 usb_device_bulk_transfer      (struct usb_device *device,        int endpoint,           void* buffer,
...
#define MAX_LEN 65536 // 16384 //8192  // 65536
	...
	while (continue_transfer)
	{
		...
		if (ms_duration > 400)
			loge("ms_duration: %d dir: %s  len: %d  bytes_xfrd: %d  total_bytes_xfrd: %d  usb_err: %d (%s)  errno: %d (%s)", ms_duration, dir, len, bytes_xfrd, total_bytes_xfrd, usb_err,
		...
		{       // If bytes were transferred but we had a timeout...
			...
			loge("Done dir: %s  len: %d  bytes_xfrd: %d  total_bytes_xfrd: %d  usb_err: %d (%s)  errno: %d (%s)", dir, len, bytes_xfrd, total_bytes_xfrd, usb_err, iusb_error_get(usb_err),
			     errno, strerror(errno));
		else if (ena_log_verbo && usb_err != LIBUSB_ERROR_TIMEOUT) // && (ena_hd_tra_send || ep == iusb_ep_in))
			logd("Done dir: %s  len: %d  bytes_xfrd: %d  total_bytes_xfrd: %d  usb_err: %d (%s)  errno: %d (%s)", dir, len, bytes_xfrd, total_bytes_xfrd, usb_err, iusb_error_get(usb_err),
			     errno, strerror(errno));
		else if (ena_log_extra)
			logw("Done dir: %s  len: %d  bytes_xfrd: %d  total_bytes_xfrd: %d  usb_err: %d (%s)  errno: %d (%s)", dir, len, bytes_xfrd, total_bytes_xfrd, usb_err, iusb_error_get(usb_err),
			     errno, strerror(errno));
	}

	if (total_bytes_xfrd > 16384)
	{       // Previously caused problems that seem fixed by raising USB Rx buffer from 16K to 64K
...
int iusb_control_transfer(libusb_device_handle *usb_hndl, uint8_t req_type, uint8_t req_val, uint16_t val, uint16_t idx, byte *buf, uint16_t len, unsigned int tmo)
{
	...
	if (usb_err < 0)
	{
		loge("Error usb_err: %d (%s)  usb_hndl: %p  req_type: %d  req_val: %d  val: %d  idx: %d  buf: %p  len: %d  tmo: %d", usb_err, iusb_error_get(usb_err), usb_hndl, req_type, req_val, val,
		     idx, buf, len, tmo);
		return (-1);
	}
	// if (ena_log_verbo)
...
int iusb_init(byte ep_in_addr, byte ep_out_addr)
{
	...
	{ // For all USB devices...
		...
		if (vendor)
		{
			...
			if (iusb_best_vendor_priority <= vendor_priority)
			{ // For last
			...
			{ // For all endpoints...
				...
				{ // 5
					if ((epdesc->bmAttributes & LIBUSB_TRANSFER_TYPE_MASK) == LIBUSB_TRANSFER_TYPE_BULK)
					{
						...
						{ // If we have both endpoints now...
							libusb_free_config_descriptor(config);

							if (ep_in_addr != 255 && (iusb_ep_in != ep_in_addr || iusb_ep_out != ep_out_addr))
							{
								logw("MISMATCH Done endpoint search iusb_ep_in: 0x%02x  iusb_ep_out: 0x%02x  ep_in_addr: 0x%02x  ep_out_addr: 0x%02x", iusb_ep_in,
								     iusb_ep_out, ep_in_addr, ep_out_addr); // Favor libusb over passed in
							}
							else
								logd("Match Done endpoint search iusb_ep_in: 0x%02x  iusb_ep_out: 0x%02x  ep_in_addr: 0x%02x  ep_out_addr: 0x%02x", iusb_ep_in,
								     iusb_ep_out, ep_in_addr, ep_out_addr);
