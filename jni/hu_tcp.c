...
int itcp_bulk_transfer(int ep, byte *buf, int len, int tmo)
{ // 0 = unlimited timeout
...
#define LIBtcp_ERROR_TIMEOUT -2
	...
	{       // Previously caused problems that seem fixed by raising tcp Rx buffer from 16K to 64K
...
int hu_tcp_recv(byte *buf, int len, int tmo)
{
	...
	int avail = 0;
	while (tmo == -1 && avail < len && ms_get() < ms_tmo)
	{						     // If tmo == -1 for special "get exactly this many bytes" function
...
int itcp_init(byte ep_in_addr, byte ep_out_addr)
{
	...
	errno = 0;
	if ((tcp_so_fd = socket(CS_FAM, CS_SOCK_TYPE, 0)) < 0)
	{ // Create socket
	...
	if (wifi_direct)
	{
		...
		errno = 0;
		if (bind(tcp_so_fd, (struct sockaddr *)&srv_addr, srv_len) < 0)
		...
		{
			int ret = system("su -c \"am start -a android.nfc.action.NDEF_DISCOVERED -t application/com.google.android.gms.car -n com.google.android.gms/.car.FirstActivity -f 32768 "
					 "1>/dev/null 2>/dev/null\""); // !! Binaries like ssd that write to stdout cause C system() to crash !
	...
	// tcp_io_fd = -1;
	while (tcp_io_fd < 0)
	{			  // While we don't have an IO socket file descriptor...
