...
unsigned int iaad_vint_dec(int log_dmp, unsigned char *buf, unsigned int len, unsigned long *vintp)
{ // Varint decode
	...
	adj = 1;
	while (rmv < 10 && lft > 0)
	{ // Assumed unsigned, 9 bytes = 63 bits
...
unsigned int iaad_dmp_arry(int log_dmp, unsigned int n, unsigned char *buf, unsigned int len)
{
	...
	if (adj != alen || adj > 8192)
	{ //  "|| rmv + adj != len) {" only works if array is last item in packet
...
unsigned int iaad_dmp_n(int log_dmp, unsigned int n, unsigned char *buf, unsigned int len)
{
	...
	{ // While bytes left to process...
		...
		if ((((unsigned int)buf[0]) & 0x07) == 0)
		{						   // If varint...
			adj = iaad_dmp_vint(log_dmp, n, buf, lft); // Adjust past the varint
		}

		else if ((((unsigned int)buf[0]) & 0x07) == 2)
		{						   // If array...
...
{ // Source:  HU = HU > AA   AA = AA > HU
	...
	*/
	...
	if (msg_type_enc_get(msg_type) == NULL)
	{		      // If encrypted data...
		return (len); // Done
	}
	...
	if (lft < 2)
	{ // If less than 2 bytes for content (impossible if content; need at least 1 byte for 0x08 and 1 byte for varint)
