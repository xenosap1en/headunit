...
int usb_device_removed(const char *devname, void *client_data)
{
	...
	{ // If we have a current device...
		...
		if (!strcmp(curr_devname, devname))
		{ // If current device was removed...
...
int usb_device_added(const char *devname, void *client_data)
{
	...
	{ // If we already have a device, just ignore
		logd("Ignoring have current_device: %p", current_device);
		usb_device_close(device);
		return (0); // 0 = Leave usb_host_run() running
	}

	if (usb_vid != USB_VID_GOO || usb_pid >= USB_PID_ACC_MIN || usb_pid <= USB_PID_ACC_MAX)
	{ // If not in Google Accessory mode...
...
int hu_usb_run()
{ // Android specific C level libusbhost API requires thread for it's processing, and calling of the added/removed/disc-done callbacks
...
void *hu_usb_thread(void *arg) { hu_usb_run(); }
int hu_usb_start()
{
	int ret = 0;
	pthread_t th;
