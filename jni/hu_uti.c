...
char *user_char_dev_get(char *dir_or_dev, int user)
{
	...
	errno = 0;
	if ((dp = opendir(dir_or_dev)) == NULL)
	{ // Open the directory. If error...
...
{ // Return 1 if file, or directory, or device node etc. exists and we can open it
...
static const char *error_code_str[ERROR_CODE_NUM + 1] = {
    "Success",						    "Unknown HCI Command",			    "Unknown Connection Identifier",
    "Hardware Failure",					    "Page Timeout",				    "Authentication Failure",
    "PIN or Key Missing",				    "Memory Capacity Exceeded",			    "Connection Timeout",
    "Connection Limit Exceeded",			    "Synchronous Connection to a Device Exceeded",  "ACL Connection Already Exists",
    "Command Disallowed",				    "Connection Rejected due to Limited Resources", "Connection Rejected due to Security Reasons",
    "Connection Rejected due to Unacceptable BD_ADDR",      "Connection Accept Timeout Exceeded",	   "Unsupported Feature or Parameter Value",
    "Invalid HCI Command Parameters",			    "Remote User Terminated Connection",	    "Remote Device Terminated Connection due to Low Resources",
    "Remote Device Terminated Connection due to Power Off", "Connection Terminated by Local Host",	  "Repeated Attempts",
    "Pairing Not Allowed",				    "Unknown LMP PDU",				    "Unsupported Remote Feature / Unsupported LMP Feature",
    "SCO Offset Rejected",				    "SCO Interval Rejected",			    "SCO Air Mode Rejected",
    "Invalid LMP Parameters",				    "Unspecified Error",			    "Unsupported LMP Parameter Value",
    "Role Change Not Allowed",				    "LMP Response Timeout",			    "LMP Error Transaction Collision",
    "LMP PDU Not Allowed",				    "Encryption Mode Not Acceptable",		    "Link Key Can Not be Changed",
    "Requested QoS Not Supported",			    "Instant Passed",				    "Pairing with Unit Key Not Supported",
    "Different Transaction Collision",			    "Reserved",					    "QoS Unacceptable Parameter",
    "QoS Rejected",					    "Channel Classification Not Supported",	 "Insufficient Security",
    "Parameter out of Mandatory Range",			    "Reserved",					    "Role Switch Pending",
    "Reserved",						    "Reserved Slot Violation",			    "Role Switch Failed",
    "Extended Inquiry Response Too Large",		    "Simple Pairing Not Supported by Host",	 "Host Busy - Pairing",
};
...
int pid_get(char *cmd, int start_pid)
{
	...
	{ // Open the /proc directory. If error...
	...
	{ // For all files/dirs in this directory... (Could terminate with errno set !!)
		...
		if (pid < start_pid)
		{ // Ignore PIDs we have already checked. Depends on directories in PID order which seems to always be true
		...
		{ // If this is a directory...
			...
			if (ret >= cmd_len)
			{ // Eg: ret = strlen ("/bin/a") = 6, cmd_len = strlen ("a") = 1, compare 1 at cmd_line[5]

				if (!strcmp(cmd, &cmdline[ret - cmd_len]))
				{ // If a matching process name
...
{ // Kill all OTHER instances of named process, except our own, if the same
	int ret = 0;
	int pid = 0;
	int our_pid = getpid();
	...
	for (idx = 0; idx < max_kill_attempts; idx++)
	{ // For maximum kills...
...
{ // Get tail buffer to write to
	...
	if (bufs >= num_aud_buf_bufs - 1)
	{ // If room for another (max = NUM_aud_buf_BUFS - 1)
...
{ // Get tail buffer to write to
	...
	if (bufs >= num_vid_buf_bufs - 1)
	{ // If room for another (max = NUM_vid_buf_BUFS - 1)
...
#define CS_DGRAM_UNIX
	...
	if (sockfd < 0)
	{
		...
		{ // Get an ephemeral, unbound socket
...
// Send cmd_buf and get res_buf
	...
	if (written != cmd_len)
	{ // Dgram buffers should not be segmented
	...
	if (written != cmd_len)
	{ // Small buffers under 256 bytes should not be segmented ?
...
int gen_server_loop(int net_port, int poll_ms)
{ // Run until gen_server_exiting != 0, passing incoming commands to gen_server_loop_func() and responding with the results
...
// Get command from client
	...
	errno = 0;
	if (listen(sockfd, 5))
	{ // Backlog= 5; likely don't need this
...
// logd ("ms_get: %d",ms_get ());
		...
		if (cmd_len <= 0)
		{
			...
			{ // Else if some other error, sleep it off for 100 ms
				if (errno == EINTR)
...
// Send response
		...
		{ // Write, if can't write full buffer...
