...
int jni_aa_cmd(int cmd_len, char *cmd_buf, int res_max, char *res_buf)
{
	...
	{ // If onCreate() hu_tra:transport_start()
	...
	/* Functions                code                            Params:
	  ...
	  Returns:
	    ...
	    Audio
	*/
	else if (cmd_buf != NULL && cmd_len >= 4)
	{ // If encrypted command to send...
		...
		if (cmd_buf != NULL && cmd_len >= 8 && cmd_buf[5] == 15)
		{ // If byebye...
			logd("Byebye");
			ms_sleep(100);
			ret = hu_aap_stop();
	...
	{ // If no queued audio or video...
		ret = hu_aap_mic_get();
		if (ret >= 1)
		{		      // && ret <= 2) {                                  // If microphone start (2) or stop (1)...
